# 50 Interview Problems 🎯

This repository contains a list of problems I have solved from **LeetCode**, **CodeChef**, and **Codeforces**. 

---

## 🟢 LeetCode (17 Problems)
1. LeetCode 1
2. LeetCode 14
3. LeetCode 27
4. LeetCode 88
5. LeetCode 121
6. LeetCode 125
7. LeetCode 169
8. LeetCode 217
9. LeetCode 219
10. LeetCode 242
11. LeetCode 283
12. LeetCode 344
13. LeetCode 349
14. LeetCode 387
15. LeetCode 448
16. LeetCode 977
17. LeetCode 2248

---

## 🟤 CodeChef (10 Problems)
1. Chef and Consecutive Ones
2. Compress the Video
3. Difficulty Rating Order
4. Find maximum in an Array
5. Largest and Second Largest
6. MIN To MAX
7. Red Light Green Light
8. Search an element in an array
9. The Squid Game
10. World Chess Championship

---

## 🔵 Codeforces (5 Problems)
1. AND, OR, Sort
2. Falling Concrete
3. Good Contest
4. Team
5. Three Piles
